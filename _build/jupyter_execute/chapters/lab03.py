...
# ```
# This uses `np.where` to do some if/else-style logic and create the new variable. You'll see the new column in your DataFrame. You can make this a **categorical** variable using `as.type`, which we've see before:
# 
# ```
...
# 
# Finally, make the plot look good with titles, labels, etc. Change for format of both the x and the y axes. You want dollar signs and commas on the y and commas in the x. Make the dots smaller using `s=`. See the `scatter()` [documentation](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.scatter.html).
# 
