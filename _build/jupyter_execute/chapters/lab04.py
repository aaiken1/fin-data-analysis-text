...
# 
# Then, use a `.groupby()` and `.cumprod()` to create cumulative returns by security. Note that you'll need that gross return to do this. The formula above shows you why.
...
# 
# First, we need to make sure the data are indexed correctly. **Use `.set_index()` to make *date* the index. Use `inplace=` to save this change.**
# 
# Then, use a `.groupby()` and `pandas` `.plot()` to graph all three cumulative returns on the same graph. Finally, graph the cumulative returns on three sub-graphs. Include a legend and a title. 
# 
# Now, reset the index on your DataFrame using .reset_index(). We are going to use a new `pandas` method called `.pivot_table()`. With this method, you select the index, the new columns, and the values you want. In other words, you are going to **spread this data from long to wide**. Our date is the index. We want each ticker to get a column. And we want the cumulative returns for each ticker. Modify this code:
# 
# ```
# dsf.pivot_table(index='_____', columns='_____', values='_____')
# ```
# 
# Do you see what it did? Now, **plot this pivoted data using `pandas` `plot()`, but have a separate plot for each ticker.** We needed to pivot the data to make the separate subplots easy to do.
# 
