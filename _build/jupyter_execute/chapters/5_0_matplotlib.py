...
# # matplotlib
# 
# We will start with Chapter 7 of our textbook and look at how to use [matplotlib](https://matplotlib.org) to create graphics in Python. `matplotlib` plays well with `numpy`, so this is a good place to start. 
# 
...
# 
# That was all using the `plot()` function that comes with `pandas` DataFrames. Let's turn our `pandas` DataFrames into **series** and look at `matplotlib` and creating figures more generally. `matplotlib` really wants to use arrays or series. No problem!
# 
...
# - Show it
# 
# Let's go back to the line graph, but do things in separate steps. This is sometimes called the [pyplot](https://matplotlib.org/stable/tutorials/introductory/usage.html#sphx-glr-tutorials-introductory-usage-py) way of creating figures.
# 
# I am going to pull the data that I want out of the DataFrame and create two new **series** called **aapl** and **msft**. These live independently of the **stocks** DataFrame. I am then going to build my plot step-by-step. It is easier to customize things this way. I will also pull the date index out of each series to use for the x-axis. Finally, I will plot two series together!
# 
...
plt.plot(aapl.index, aapl, alpha = 0.5, label = 'AAPL', color = 'red')
plt.plot(msft.index, msft, alpha = 0.5, label = 'MSFT', color = 'blue')
plt.xlabel('Date')
plt.ylabel('Price')
...
plt.subplot(211)
plt.plot(aapl.index, aapl, lw=1.5, label='AAPL')
plt.legend(loc=0)
plt.ylabel('Price')
plt.title('AAPL vs. MSFT')

plt.subplot(212)
plt.plot(msft.index, msft, 'g', lw=1.5, label='MSFT')
plt.legend(loc=0)
plt.xlabel('Date')
plt.ylabel('Price');
...
msft_ret = np.log(msft / msft.shift(1))  
...
# I like how the default colors are the NY Mets for some reason. Notice how I passed `plt.hist` both series together.
...
# 
# This first step creates a blank `figure` object. We actually did this above too. A `figure` is going to have special properties that we can do stuff with.
# 
# We are also going to create `ax` objects. 
# 
# You can also think about the different parts of the plot as different objects to change.
...
# ```
# 
# Let's go back and just create a single plot using this `figure` and `axis` object logic. Again, you can think of this as just adding pieces of the figure together. We start with the canvas and how big it is. We add a plot. And then we style things. I even add some random text.
...
# 
# We can use the same `figure` and `axis` logic to just a 2 x 2 grid of graphs. This is where you can really start to see the power of this. I added some colors so that you can see which plot goes where in the grid. I'm using some random numbers just to demonstrate. I'm also adjusted the white space around the grid.
...
axes[1,0].hist(np.random.randn(500), bins=50, color='b', alpha=0.5)
axes[0,1].hist(np.random.randn(500), bins=50, color='r', alpha=0.5)
axes[1,1].hist(np.random.randn(500), bins=50, color='g', alpha=0.5)
